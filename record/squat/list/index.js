function init() {
  getRecordData();
}

function getRecordData() {
  loadingOn();
  let url = window.location.pathname;
  url = url.split("/");
  const record_type = url[2];
  const page = getParam("page");
  const search_key = getParam("search_key");
  const search_keyword = getParam("search_keyword");

  $.ajax({
    type: "POST",
    data: JSON.stringify({
      record_type: record_type,
      page: page,
      search_key: search_key,
      search_keyword: search_keyword,
    }),
    url: "/api/record/get.record_board_list_data.php",
    success: function (data) {
      console.log(data);
      if (data["code"] == "SUCCESS") {
        let renderHtml = "";
        const listData = data["data"];
        for (key in data["data"]) {
          renderHtml +=
            '<div class="board_row">' +
            '<div class="body_box writer">' +
            '<a href="/record/record_view/?record_id=' +
            listData[key]["record_id"] +
            '">' +
            listData[key]["nickname"] +
            "</a>" +
            "</div>" +
            '<div class="body_box weight">' +
            '<a href="/record/record_view/?record_id=' +
            listData[key]["record_id"] +
            '">' +
            listData[key]["record_weight"] +
            "</a>" +
            "</div>" +
            '<div class="body_box audit">' +
            '<a href="/record/record_view/?record_id=' +
            listData[key]["record_id"] +
            '">' +
            listData[key]["record_status"] +
            "</a>" +
            "</div>" +
            '<div class="body_box date">' +
            listData[key]["date"] +
            "</div>" +
            "</div>";
        }

        $("#board_wrap .board_container .board_body_wrap").html(renderHtml);

        pageRender(data["page"]);

        if (search_key != "") {
          $("#search_key option[value=" + search_key + "]").prop(
            "selected",
            true
          );
        }
        if (search_keyword != "") {
          $("#search_keyword").val(decodeURI(search_keyword));
        }
      } else if (data["code"] == "EMPTY") {
        //EMPTY
        var alertEle = '<div class="empty_box">' + data["msg"] + "</div>";
        $("#board_wrap .board_container .board_body_wrap").append(alertEle);
      } else {
        myrecordAlert("on", data["msg"]);
      }
      loadingOff();
    },
    error: function (error) {
      loadingOff();
      console.log(error);
    },
  });
}

function pageRender(data) {
  let pageHtml = "";
  let page = getParam("page");
  if (page == "") {
    page = 1;
  }
  for (key in data) {
    if (page == data[key]) {
      pageHtml +=
        '<button class="page_btn select" onclick="pageMove(' +
        data[key] +
        ')">' +
        data[key] +
        "</button>";
    } else {
      pageHtml +=
        '<button class="page_btn" onclick="pageMove(' +
        data[key] +
        ')">' +
        data[key] +
        "</button>";
    }
  }

  $(".paging_wrap .paging_box").html(pageHtml);
  console.log(data);
}

function pageMove(num) {
  let prev_param = window.location.search;
  const page_check = getParam("page");
  let next_param = "";
  if (page_check === "") {
    if (prev_param === "") {
      // 기존 파라미터가 없을 경우
      next_param = prev_param + "?page=" + num;
    } else {
      // 기존 파라미터가 있을 경우
      next_param = prev_param + "&page=" + num;
    }
  } else {
    next_param = prev_param.replace(/page=[0-9]/g, "page=" + num);
  }

  location.href = window.location.pathname + next_param;
}

function prevPage() {
  let prev_param = window.location.search;
  const page_check = getParam("page");
  let next_param = "";
  let page = "";
  if (page_check == "") {
    page = 1;
    next_param = prev_param + "&page=" + page;
  } else {
    page = parseInt(page_check) - 1;
    if (page < 1) {
      page = 1;
    }
    next_param = prev_param.replace(/page=[0-9]/g, "page=" + page);
  }

  location.href = window.location.pathname + next_param;
}

function nextPage() {
  let prev_param = window.location.search;
  const page_check = getParam("page");
  let next_param = "";
  let page = "";
  if (page_check == "") {
    page = 1;
    next_param = prev_param + "&page=" + page;
  } else {
    page = parseInt(page_check) + 1;
    if (page < 1) {
      page = 1;
    }
    next_param = prev_param.replace(/page=[0-9]/g, "page=" + page);
  }

  location.href = window.location.pathname + next_param;
}

function recordSearch() {
  const search_key = $("#search_key").val();
  const search_keyword = $("#search_keyword").val();

  location.href =
    window.location.pathname +
    "?search_key=" +
    search_key +
    "&search_keyword=" +
    search_keyword;
}
