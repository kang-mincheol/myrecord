...
echo css_load('/workout_log/write/index.css');   // 작성 페이지 CSS 재사용
...
<div class="workout_write_wrap">
        ...
        <div class="form_row">
            ...
            <div class="input_with_unit">
                <input type="number" id="workout_duration" class="form_input" placeholder="운동한 시간을 입력해주세요" min="1" max="1440"
                    value="<?= htmlspecialchars($log['workout_duration'] ?? '') ?>" />
    ...
    <div class="form_card" id="exercise_card">
        ...
        <div id="exercise_list">
            <?php foreach($exercises as $exIdx => $ex): ?>
            <div class="exercise_block" data-index="<?= $exIdx ?>">
                <div class="exercise_block_header">
                    <div class="exercise_name_wrap">
                        <input type="text" class="exercise_name_input"
                               placeholder="운동 종목명 (예: 스쿼트, 벤치프레스)"
                               maxlength="100"
                               value="<?= htmlspecialchars($ex['exercise_name']) ?>" />
                    </div>
                ...
                <div class="set_list">
                    <?php foreach($ex['sets'] as $sIdx => $set): ?>
                    <div class="set_row">
                        <span class="set_no_label"><?= $sIdx + 1 ?>set</span>
                        <div class="set_input_group">
                            ...
                            <div class="set_input_wrap">
                                <input type="number" class="set_reps_input"
                                       placeholder="0" min="0" max="9999"
                                       value="<?= htmlspecialchars($set['reps']) ?>" />
    ...
    <div class="form_card">
        ...
        <textarea id="workout_memo" class="form_textarea"
                  ...
                  maxlength="500"
                  oninput="memoCount(this);"><?= htmlspecialchars($log['memo'] ?? '') ?></textarea>
        <div class="memo_count_wrap"><span id="memo_count"><?= mb_strlen($log['memo'] ?? '') ?></span> / 500</div>
    </div>
