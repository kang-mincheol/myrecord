      ...
      if (data["code"] == "SUCCESS") {
        if (data["data"]["team"]) {
          ...
          for (var key in team) {
            team_html +=
              ...
              '<div class="label_box">' +
              '<label class="wrap_label" for="team_name_' +
              team[key]["id"] +
              '">' +
              (parseInt(key) + 1) +
              " 팀명</label>" +
              ...
              '<input id="team_name_' +
              team[key]["id"] +
              '" class="input_text" onkeyup="inputOnkeyupEvent(this);" type="text" value="' +
              team[key]["team_name"] +
              '"/>' +
              "</div>" +
              "</div>";
            ...
            score_html +=
              '<div class="myrecord_input_wrap">' +
              '<div class="label_box">' +
              '<label class="wrap_label" for="team_score_' +
              team[key]["id"] +
              '">' +
              team[key]["team_name"] +
              " 팀</label>" +
              "</div>" +
              '<div class="form_value_box">' +
              '<input id="team_score_' +
              team[key]["id"] +
              '" class="input_text" onkeyup="inputOnkeyupEvent(this);" type="number" value="0"/>' +
              "</div>";
            ("</div>");
            ...
            person_html +=
              ...
              '<div class="myrecord_input_wrap">' +
              '<div class="label_box">' +
              '<label class="wrap_label" for="add_person_' +
              team[key]["id"] +
              '">' +
              team[key]["team_name"] +
              " 팀</label>" +
              "</div>" +
              '<div class="form_value_box">' +
              '<input id="add_person_' +
              team[key]["id"] +
              '" class="input_text" onkeyup="inputOnkeyupEvent(this);" type="text"/>' +
              "</div>" +
              "</div>" +
              '<button class="add_btn" onclick="setTeamPerson(' +
              team[key]["id"] +
              ')">추가</button>' +
              "</div>";
          }
        ...
        if (data["data"]["score"] && data["data"]["score"].length > 0) {
          ...
          for (var key in score) {
            $("#team_score_" + score[key]["team_id"]).val(
              score[key]["team_score"]
            );
          }
        }
        ...
        if (data["data"]["person"] && data["data"]["person"].length > 0) {
          ...
          for (var key in person) {
            person_html +=
              '<div class="person_box" person_id="' +
              person[key]["id"] +
              '" onclick="deletePerson(' +
              person[key]["id"] +
              ');">' +
              person[key]["person_name"] +
              "</div>";
          }
...
function setTeamMaster() {
  ...
  for (var i = 1; i <= 3; i++) {
    ...
    if (teamName == "") {
      myrecordAlert("on", "팀 이름을 입력해 주세요");
      return;
    }
    teamData[i] = teamName;
  }
...
function setTeamScore() {
  var scoreData = {};
  var length = $(
    "#team_score_wrap .team_score_wrap .myrecord_input_wrap"
  ).length;
